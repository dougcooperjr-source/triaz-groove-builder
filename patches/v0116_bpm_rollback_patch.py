...
fold_replacement = '''        a, b = v[:32], v[32:]
        ah, bh = a > 1e-7, b > 1e-7
        both = ah & bh
        only_a = ah & ~bh
        only_b = bh & ~ah
...
'''
fold_func = s.index('def fold_four_bars_to_one_pattern')
start = s.index('        a, b = v[:32], v[32:]', fold_func)
end = s.index('        out_lanes[ch] = LanePattern', start)
s = s[:start] + fold_replacement + s[end:]
